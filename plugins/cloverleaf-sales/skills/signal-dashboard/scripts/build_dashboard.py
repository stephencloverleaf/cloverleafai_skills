...
Accepts ANY of:
  • raw Cloverleaf transcript `search-meetings` output -> {"results": [...]}
  • raw `run-document-keyword-search` output   -> {"object_api_response": {...}}  (procurement)
...
Normalized signal schema (all fields optional except quote + jurisdiction):
  {
    ...
    "video_url": "https://.../media.mp4",   # OPTIONAL fallback only; the card links to the
                                             # Cloverleaf meeting page (built from meeting_id) by default
    "cloverleaf_url": "https://app.cloverleaf.ai/meetings/18451",  # optional explicit override
...
Procurement signals usually have NO speaker — they're scored on dollar figure, stage,
...
SKY  = "#A9E3F4"   # Sky-deep — official signature accent. Cloverleaf brand has NO green.
...
def normalize_raw_search(payload):
    """Collapse raw Cloverleaf search results into one best signal per meeting."""
    out = []
...
def ingest(payload):
    ...
    raise SystemExit("Unrecognized JSON. Expected transcript search {results:[...]}, "
                     "document search {object_api_response:...}, {signals:[...]}, or a bare list.")
...
def _passages(hit):
    hl = (hit.get("highlight") or {}).get("plain_text") or []
    # strip the <mark> tags for clean display; keep order
    return [re.sub(r"</?mark>", "", p).strip() for p in hl if p and p.strip()]
...
def normalize_raw_documents(payload):
    ...
    procurement-language matches are dropped — mirrors the document-signal-search skill's
...
def card(sig):
    ...
    if st:
        try:
            ...
            ts_hint = ""
    watch = ""
    # Always send the rep to the Cloverleaf platform meeting page — that's where the video
    # AND the AI insights live. Build it from the meeting id (or use an explicit
    # `cloverleaf_url` override). Only fall back to a raw source video when there's no id.
    cl_url = sig.get("cloverleaf_url")
    if not cl_url and sig.get("meeting_id"):
        _mid = _meeting_num(sig["meeting_id"])
        if _mid:
            cl_url = f"https://app.cloverleaf.ai/meetings/{_mid}"
    if cl_url:
